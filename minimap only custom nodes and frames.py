...
def analyze_nodes(context):
    ...
    # Filter nodes - only keep those with labels
    nodes = [n for n in tree.nodes if hasattr(n, 'label') and n.label and n.label.strip()]
    if not nodes:
        return None
...
def draw_minimap(layout, analysis_result):
    if analysis_result is None:
        layout.label(text="No renamed nodes to display")
        return
    ...
    for row in range(GRID_ROWS):
        ...
        for col in range(GRID_COLS):
            ...
            if nodes_in_cell:
                nd = nodes_in_cell[0]  # Only one node per cell since we filtered
